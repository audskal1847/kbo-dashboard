...
def display_local_excel(title, file_name):
    ...
    try:
        ...
        # 🧹 [세탁 1] 'Unnamed'가 들어간 열 강제 삭제
        cols_to_drop = [col for col in df.columns if 'Unnamed' in str(col)]
        df.drop(columns=cols_to_drop, inplace=True)
        
        # 🧹 [세탁 2] '.1' 제거 및 에러 방지 (눈에 안 띄는 띄어쓰기 마법!)
        new_columns = []
        for col in df.columns:
            clean_col = str(col).replace('.1', '')
            # 만약 이름이 이미 존재한다면, 뒤에 띄어쓰기(" ")를 붙여서 컴퓨터가 다른 기둥으로 인식하게 속임
            while clean_col in new_columns:
                clean_col += " "
            new_columns.append(clean_col)
        df.columns = new_columns
        
        # 🧹 [세탁 3] 지저분한 None, NaN 값들을 완전히 빈칸("")으로 교체
        df = df.fillna("")
            
        st.dataframe(df, use_container_width=True)
        
    except FileNotFoundError:
        st.error(f"⚠️ '{file_name}' 파일을 찾을 수 없습니다! 깃허브에 파일이 올바르게 업로드되었는지 확인해주세요.")
    except Exception as e:
