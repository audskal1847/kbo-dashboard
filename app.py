...
# ==========================================
# 5. 공통 엑셀 파일 업로드 및 표시 함수 (⭐ 기억력 향상!)
# ==========================================
def handle_excel_upload(title, description, key):
    st.title(title)
    st.markdown(description)
    
    uploaded_file = st.file_uploader("엑셀 파일(.xlsx)을 업로드하세요", type=["xlsx", "xls"])
    
    # 새로운 파일이 업로드되면, 프로그램의 '임시 기억장치(session_state)'에 꽉 저장해 둡니다.
    if uploaded_file is not None:
        try:
            xls = pd.ExcelFile(uploaded_file)
            st.session_state[f"data_{key}"] = pd.read_excel(uploaded_file, sheet_name=None)
            st.session_state[f"sheets_{key}"] = xls.sheet_names
            st.success("✅ 파일 저장 완료! 이제 다른 메뉴에 다녀와도 기록이 사라지지 않습니다.")
        except Exception as e:
            st.error(f"엑셀 파일을 읽는 중 오류가 발생했습니다: {e}")
            
    # 기억장치에 저장된 데이터가 있는지 확인하고, 있으면 화면에 계속 띄워줍니다.
    if f"data_{key}" in st.session_state:
        sheet_names = st.session_state[f"sheets_{key}"]
        all_data = st.session_state[f"data_{key}"]
        
        if len(sheet_names) > 1:
            selected_sheet = st.selectbox("기록을 확인할 시트(팀)를 선택하세요:", sheet_names, key=f"sel_{key}")
            df = all_data[selected_sheet]
            st.subheader(f"📂 {selected_sheet} 데이터")
        else:
            df = all_data[sheet_names[0]]
...
if menu == "🔍 도윤이의 선수 검색기":
    ...
    st.markdown("선수 이름(예: 박건우, 양현종)을 입력하면 1군 기록이 있는 동명이인을 모두 찾아 보여줍니다.")
    ...
    fetch_and_display_data("🏟️ KBO 리그 구단 소개", "https://www.koreabaseball.com/Kbo/League/TeamInfo.aspx", "구단 정보를 불러오는 중입니다...")

elif menu == "📈 구단 변천사":
    fetch_and_display_data("📈 KBO 리그 구단 변천사", "https://www.koreabaseball.com/Kbo/League/TeamHistory.aspx", "구단 변천사 정보를 불러오는 중입니다...")

elif menu == "📊 팀 간 상대전적 (엑셀)":
    handle_excel_upload("📊 팀 간 상대전적", "정리하신 **[팀 간 상대전적.xlsx]** 파일을 아래에 드래그하여 업로드해주세요.", "excel_vs")

elif menu == "📅 연도별 종합성적 (엑셀)":
    handle_excel_upload("📅 연도별 종합성적", "정리하신 **[연도별 종합성적.xlsx]** 파일을 아래에 드래그하여 업로드해주세요.", "excel_yearly")

elif menu == "🏅 역대 통산성적 (엑셀)":
    handle_excel_upload("🏅 역대 통산성적", "정리하신 **[통산성적.xlsx]** 파일을 아래에 드래그하여 업로드해주세요.", "excel_total")
