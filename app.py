...
def display_local_excel(title, file_name):
    ...
    try:
        ...
        # 🧹 [데이터 세탁 로직 1] 'Unnamed'가 들어간 불필요한 열(예: Unnamed: 5) 삭제
        cols_to_drop = [col for col in df.columns if 'Unnamed' in str(col)]
        df.drop(columns=cols_to_drop, inplace=True)
        
        # 🧹 [데이터 세탁 로직 2] 열 이름에 붙은 꼴보기 싫은 '.1' 강제 제거
        df.columns = [str(col).replace('.1', '') for col in df.columns]
        
        # 🧹 [데이터 세탁 로직 3] 표 안의 'None'이나 'NaN' 값을 모두 깔끔한 빈칸("")으로 변경
        df = df.fillna("")
