...
with st.sidebar:
    ...
    menu = st.radio(
        ...
        [
            "🔍 도윤이의 선수 검색기", 
            "🏆 정규시즌 팀 순위", 
            "📊 팀 간 상대전적 (엑셀)",  # ⭐ 추가된 엑셀 메뉴 1
            "📅 연도별 종합성적 (엑셀)", # ⭐ 추가된 엑셀 메뉴 2
            "🏅 역대 통산성적 (엑셀)",  # ⭐ 추가된 엑셀 메뉴 3
            "🏃 타자 순위 (Top 30)", 
            "⚾ 투수 순위 (Top 30)", 
            "🏟️ 관중 현황",
            "📜 역대 기록 (구단)", 
...
# ==========================================
# 5. 공통 엑셀 파일 업로드 및 표시 함수 (⭐ 추가됨)
# ==========================================
def handle_excel_upload(title, description, key):
    st.title(title)
    st.markdown(description)
    
    uploaded_file = st.file_uploader("엑셀 파일(.xlsx)을 업로드하세요", type=["xlsx", "xls"], key=key)
    ...
    if uploaded_file is not None:
        try:
            ...
            # 시트가 여러 개일 경우 선택할 수 있게 함 (팀별 구분용)
            ...
            st.error(f"엑셀 파일을 읽는 중 오류가 발생했습니다. (오류 내용: {e})")
